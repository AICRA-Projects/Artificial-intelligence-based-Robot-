...
out_b = 13
out_c = 11
GPIO.setup(out_a,GPIO.OUT)
GPIO.setup(out_b,GPIO.OUT)
GPIO.setup(out_c,GPIO.OUT)

faceCascade = cv2.CascadeClassifier('/home/aicra/Documents/opencv-master/data/haarcascades/haarcascade_fullbody.xml')
end_time = time.time()
...
while True:
    ...
    for (x,y,w,h) in faces:
        ...
        if (x > 0 and x < 100) and (y > 0 and y < 160):
            ...
            if ab > -25 and ab < 60:
                rob = "near"
                GPIO.ouptut(out_a,LOW)
                GPIO.output(out_b,HIGH)
                GPIO.output(out_c,LOW)
                cv2.putText(img,str(rob),(x, y+20), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255,255,0), 2)
            elif ab > 60 and ab < 100:
                rob = "accept"
                GPIO.ouptut(out_a,HIGH)
                GPIO.output(out_b,LOW)
                GPIO.output(out_c,LOW)
                cv2.putText(img,str(rob),(x, y+20), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255,255,0), 2)
            else:
                rob = "reject"
